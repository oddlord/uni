...
\section*{LINFA: Smart drugs restocking}
  ...
  \subsection*{Smart drugs restocking}
    ...
    \question{Build a model of a generic ward with the above specifications and 3 days of lookahead, in order to support decision to the restock order of the current day. The arrival of patients and the consumption of drugs can  modelled through custom distributions, as well as drug restock order choices.}
    \answer{
      The model in Code \ref{lst:ward} represents a generic ward, fully described through probabilistic behaviours.
      
      \begin{center}
        \lstinputlisting[language=prism, caption={PRISM code for the model of a hospital ward.}, label={lst:ward}]{code/ward_dtmc.sm}
      \end{center}
    
      The first thing that can be seen about the model is that it is defined as a DTMC. This is due the fact that the model is fully probabilistic (i.e. there is no non-determinism) and that the time is divided into discrete steps.
